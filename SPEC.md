...
- URL-as-state sync packet (`#state=` hash, base64 JSON) with load-time apply, **A/B swap when the sender’s “who is A” differs from this phone**, **onboarding toast** after link open when names are still placeholders or don’t match the packet, hash strip, and shipped lifecycle resume listeners (`pageshow` / `hashchange` / `visibilitychange`) that re-run apply or repaint on foreground
- Messages (`sms:`) handoff after spontaneous award, after request, and after awarding a pending request (on **desktop**: `#desktop-handoff` + **Copy Message** + `#toast` celebration; Step 12 timer coordination is partially shipped: shared dismiss timer + clear-on-transition + handoff de-confliction; pre-filled SMS follows *Core Mechanics* (points, recipient, what was noticed, link — quip stays in-app))
...
- **Mobile category grid density:** CSS now collapses to **one column** below 360px viewport width (reduced from 480px), improving two-column coverage on common phones; broader staged layout options and acceptance tests still live in `BUILD-v2.md` (*Staged backlog — mobile category grid (two columns)*), with a roadmap pointer in `ROADMAP-12w.md`
...
### 2026-04-14 — desktop Messages handoff + Copy (possible bug, now partially addressed)
...
- **Implementation status (2026-04-17):** Step 12 infrastructure is now partly shipped: coordinated `toastDismissTimer`, timer clearing across toast paths, and desktop handoff suppresses competing floating toast state. **Remaining polish:** final interaction feel (e.g., inline copy confirmation on button) and device QA to confirm no lingering/competing toast perception.
